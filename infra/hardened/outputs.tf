...
output "security_group_ids" {
  value = try(module.network[0].security_group_ids, null)
}

output "private_subnet_ids" {
  value = try({
    web  = module.network[0].web_subnet_ids
    was  = module.network[0].was_subnet_ids
    data = module.network[0].data_subnet_ids
  }, null)
}

output "alb_dns_name" {
  value = try(module.edge[0].alb_dns_name, null)
}
...
output "canary_bucket_arn" {
  value = try(module.canary[0].bucket_arn, null)
}
